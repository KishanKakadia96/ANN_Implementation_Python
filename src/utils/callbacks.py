...
def get_callbacks(config, X_train):
    ...
    TENSORBOARD_ROOT_DIR = os.path.join(logs["logs_dir"], logs["TENSORBOARD_ROOT_LOG_DIR"], unique_dir_name)

    os.makedirs(TENSORBOARD_ROOT_DIR,exist_ok=True)

    tensorboard_cb = tf.keras.callbacks.TensorBoard(log_dir=TENSORBOARD_ROOT_DIR)
    
    file_writer = tf.summary.create_file_writer(logdir=TENSORBOARD_ROOT_DIR)
